...
function module.Execute(Client: Player?, Type: string, Attachment)
	if Type == "command" then
		...
		module.API.Players.hint(
			Client,
			"System",
			ok and "Successfully banned player " .. possiblyUserId .. "!"
				or "An error occured while trying to ban player "
					.. possiblyUserId
					.. "\n\nError message:\n"
					.. response
		)
...
function module.Init()
	private.DataStore = module.Services.DataStoreService:GetDataStore(
		module.Settings.Misc.DataStoresKey.Ban or "commander.bans"
	)
			...
			elseif type(data.End) == "number" and data.End > os.time() then
				...
				Player:Kick(
					"Banned by "
						...
						.. "\n\nReason: "
						.. data.Reason
						.. "\n\nNote: This ban is a timed global ban, you will not be allowed to join this game until "
						.. os.date("%c", data.End)
				)
			end
		end
